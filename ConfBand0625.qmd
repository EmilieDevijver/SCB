...
To analyze this  noisy data, a classic approach consists in applying a kernel function to smooth the noise or projecting the data onto a functional space defined by a family of functions (@LI2022104806, @kokoszka2017introduction Chapter 3). 
In both approaches, the targeted function is then an approximation of the true function and obtained confidence bands are thus bands of this approximated function. 
...
$$ Z_L(t):= \frac{Stat_{L}(t)}{\sqrt{\text{Var}(Stat_{L}(t))}} = \frac{\underline{\hat{f}}^{L}(t) - \underline{f}^{L}(t)}{\sqrt{C^{L}(t,t)}}\sim \mathcal{N}(0,1).$$
...
$$
q^L = \arg\min_{q} \left\{ \mathbb{P}\left(\max_{t \in [0,1]} \left|Z_L(t)\right| \leq q \right) = 1 - \alpha\right\}.
$$ {#eq-qL}
...
In this section, we propose a new method in the non asymptotic setting to provide a confidence band of $f^{L^*}$. 
...
To illustrate the model, we simulate a regression functional model with $n=50$ points per individual and $N=25$ individuals. In @fig-BasisModel, the function $f$ (red curve) belongs to the Fourier (resp. Legendre and Spline) family with $L^*=11$ and the noisy observations $y_{ij}$ (grey curves) have a functional noise in dimension $L^{\varepsilon}=20$, also in the Fourier (resp. Legendre and Spline) family on the left plot (resp. middle and right). 
...
ggplot(data = df, aes(x = Time, y = f, color = type, group = ind)) + 
  ...
  scale_linetype_manual(values = c("obs" = "solid", "True" = "solid")) +
  facet_grid(~basis)
...
The empirical confidence level is then evaluated as the proportion of confidence bands that contain the true function. @tbl-levelL presents the empirical confidence levels for different values of $L$ and two sample sizes $n=50$ and $n=150$, with $N=10$, $N=25$ and $N=60$. 
When $N=25$ and $N=60$, the level is the expected one whatever the value of $L$, especially when $L<L^*$ and $N=25$,  but also when $L>L^\varepsilon$. We will see in the next sections that this will not be the case for the debiased confidence band. When $N=10$, the level is too small, especially when $L$ is large. This might be due to the the large number of parameters to be estimated in the covariance matrix $C$, with a small number of observations $N$. 
...
d.L.Lmax = function(data, basis, L, Lmax=15, alpha = 0.05){
...
```{r, fig.height = 7, fig.width=7}
#| label: fig-Lstar
#| fig-cap: "Illustrative example. For a given dataset, we plot several confidence bands (top row), functions dL (middle row) and dLLmax (bottom row). Bands and functions are estimated with Fourier (left column), Legendre (middle column) and Spline (right column) basis and several dimensions L (3, 5, 11, 15)."

CB2 = function(data, basis, L, Lmax = 13, alpha = 0.05, beta = 0.05){
...
for (basis in c("Legendre", "Fourier", "Splines")){
  ...
  crit.sel = matrix(NA, ncol = 3, nrow = 16)
  for (L in vec.L){
    conf.band.L.star.2 = CB2(data, basis, L, Lmax = 13, alpha = 0.05)
  ...
  for (ind.n in 1:length(vec.n)){
    for (L in vec.L){
      ...
      for (rep in 1:nb.repeat){
        ...
        conf.band.L.star.2 = CB2(data, basis, L, Lmax = 13, alpha = 0.2, beta=0.2)
...
```{r, fig.height = 7, fig.width=7}
#| label: tbl-levLstar2
#| tbl-cap: "Illustrative example. We display the level of confidence over 100 repetitions for the proposed confidence band CB3, for several model selection criterion in rows and several basis in columns, for alpha=0.05 and n=50, N=25."
...
if (run == TRUE){
  ...
  for (basis in c("Legendre", "Fourier", "Splines")){
    ...
    if (basis == 'Fourier'){ vec.L = seq(3,12, by=2)}
...
Next, we illustrate the $\tilde L$ dimension selection method and compare it to the cross-validation method.  @tbl-levelL-3 presents the coverage rates of the corresponding confidence bands estimated with the Fourier and Legendre basis, in the case $N=25$ and $n=150$. Once again, we see that the Fourier basis does not give good results, either by cross-validation or by $\tilde L$. On the other hand, with the Legendre basis, the $\tilde L$ method gives a satisfactory coverage rate, even if it is underestimated, whereas the cross-validation method is very poor. 
    ...
    for (ind.N in 1:length(vec.N)){
      for (ind.basis in 1:2){
        ...
        for (rep in 1:nb.repeat){
          ...
          for (L in vec.L){
            CB2.L = CB2(data, basis = basis, L, Lmax = 15, alpha = sqrt(0.05), beta = sqrt(0.05))
            length.band = c(length.band, CB2.L$width)
            conf.band.L = CB1(data, basis, L, alpha = (0.05))
            c.L[L] = max(conf.band.L$c.L)
          }
...
perf.Fourier %>%
  kable("latex", booktabs = TRUE) %>%
  add_header_above(c("L" = 1, "n=150, N=25" = 1))
...
load("Res_Tab_other_2_width.RData")
ggplot(dfBandWidth, aes(x = dimension, y = Band.width)) + 
  geom_boxplot() + 
  facet_wrap(~basis, scales = "free_y")
```
...
for (basis in c("Fourier", "Legendre", "Splines")){
  if (basis == "Splines"){
    data = dgp(sd=2*1,L.star=L.star, L.eps = 15, alpha.mu = NULL, basis = basis, f.true = NULL)
  } else { data = dgp(sd=1,L.star=L.star, L.eps = 15, alpha.mu = NULL, basis = basis, f.true = NULL)
  }
  alpha.tot = c()
  for (L in vec.L){
    set.seed(1)
  ...
  axis(2, at = seq(0, 1, length = ncol(alpha.tot)), labels = c("L=7, n=40", "L=7, n=100", "L=15, n=40", "L=15, n=100"), las = 2, cex.axis = 0.6)
