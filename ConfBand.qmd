...
```{r, out.height = "25%", fig.width=7}
#| label: fig-critlast
#| fig-cap: "Illustrative example. For a given simulated dataset, we show the behavior of the criteria $\widetilde{crit}$ as a function of  dimension L, for Fourier (left), Legendre (middle) and Splines (right) basis."
#| 
...
```{r, out.height = "25%", fig.width=7}
#| label: fig-width-2
#| fig-cap: "Illustrative example. We display within boxplots the confidence band's width over 100 repetitions for the dimension $\hat L$ selected by crit, the dimension $\tilde L$ selected by $\widetilde{crit}$, some fixed Lmax and the true (unknown) $L^{*}$ for Fourier (left), Legendre (middle) and Splines (right) basis."
#| 
...
As the data is not periodic, the Fourier basis is meaningless, and so is the associated confidence band, whatever the dimension considered. Both splines and Legendre basis give similar confidence bands. 
When analyzing the width of the bands compared with the one obtained with $L_{\max}$, we see that there are less smooth but also smaller, and from our empirical study we guess that it makes a trade-off between bias and variance. 
...
First, when considering a functional family with a fixed level, we discuss the confidence band derived from @sun1994. It is biased if the level is not high enough to approximate well the true function.
