...
To illustrate the model, we simulate a regression functional model with $n=40$ regular timepoints per individual and $N=25$ individuals. In @fig-BasisModel, the function $f$ (red curve) belongs to the Fourier (resp. Legendre and Spline) family with $L^*=7$ and the noisy indiivdual observations $y_{ij}$ (grey curves) have a functional noise in dimension $L^{\varepsilon}=15$, also in the Fourier (resp. Legendre and Spline) family on the left plot (resp. middle and right). 
...
The empirical confidence level is then evaluated as the proportion of confidence bands that contain the true function. @tbl-levelL presents the empirical confidence levels for different values of $L$ and several sample sizes $n \in \{40, 150\}$, and $N \in \{10, 25, 60\}$, and for the 3 basis. 
...
```{r, fig.height = 7, fig.width=7}
#| label: fig-Lstar
#| fig-cap: "Visualization of confidence bands by correcting the bias. For a given dataset, we plot several confidence bands (top row), functions dL (middle row) and dLLmax (bottom row). Bands and functions are estimated with Fourier (left column), Legendre (middle column) and Spline (right column) basis and several dimensions L (3, 5, 7, 11, 15, 21), and Lmax = 25."
...
if (run == TRUE){
  ...
  for (basis in c( "Fourier", "Legendre", "Splines")){
    ...
    rownames(perf) = vec.L
    colnames(perf) = vec.n
    save(perf, file = paste0("Res_Tab2_", basis, ".RData"))
    kbl_obj = perf %>% kable("latex", booktabs = TRUE) %>% add_header_above(c("L" = 1, "n" = 2))
    table_list[[basis]] <- kbl_obj
    }
} 
load(paste0("Res_Tab2_Fourier.RData"))
perf.F = perf 
load(paste0("Res_Tab2_Legendre.RData"))
perf.L = perf 
load(paste0("Res_Tab2_Splines.RData"))
perf.S = perf 
...
```{r, fig.height = 7, fig.width=7}
#| label: tbl-levLstar2
#| tbl-cap: "Comparison with the state-of-the-art for the empirical confidence level. The table reports the performance of the proposed confidence band CB3, estimated over 1000 repetitions, for various model selection criteria and competitive methods (rows) and for various basis families (columns). Results are shown for alpha=0.05, with n=50 and N=25."
